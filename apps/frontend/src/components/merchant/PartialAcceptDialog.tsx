...
export function PartialAcceptDialog({
  ...
  useEffect(() => {
    ...
    setAvailableIds(lines.map((line) => line.merchant_product_id));
    setNotes('');
  }, [isOpen, lines]);
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-2xl rounded-md bg-card p-5 shadow-card"
        role="dialog"
        aria-modal="true"
        aria-labelledby="partial-accept-title"
        onClick={(event) => event.stopPropagation()}
      >
        <h2 id="partial-accept-title" className="text-lg font-black">
          Accepter partiellement la Kadhia
        </h2>
