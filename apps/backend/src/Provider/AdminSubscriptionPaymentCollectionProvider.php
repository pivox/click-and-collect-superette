 ...
 */
final readonly class AdminSubscriptionPaymentCollectionProvider implements ProviderInterface
{
    ...
    public function provide(Operation $operation, array $uriVariables = [], array $context = []): SubscriptionPaymentListOutput
    {
        ...
        $page = $this->parsePositiveInt($request?->query->get('page'), self::DEFAULT_PAGE, 'ADMIN_SUBSCRIPTION_PAYMENT_INVALID_PAGE');
        $limit = $this->parsePositiveInt($request?->query->get('limit'), self::DEFAULT_LIMIT, 'ADMIN_SUBSCRIPTION_PAYMENT_INVALID_LIMIT');
        $limit = min(self::MAX_LIMIT, $limit);
        $offset = ($page - 1) * $limit;

        $items = array_map(
            fn ($payment) => $this->outputFactory->fromSubscriptionPayment($payment),
            $this->subscriptionPaymentRepository->findPaginated($limit, $offset),
        );
        ...
        return new SubscriptionPaymentListOutput(
            ...
            page: $page,
            limit: $limit,
            total: $this->subscriptionPaymentRepository->countAll(),
        );
    }
