...
/**
 * @implements ProcessorInterface<CustomerStoreFavoriteInput, CustomerStoreOutput>
 */
final readonly class UpdateStoreFavoriteProcessor implements ProcessorInterface
{
    ...
    {
        if (!$data instanceof CustomerStoreFavoriteInput) {
            throw new \InvalidArgumentException('CustomerStoreFavoriteInput expected.');
        }
        ...
        if (null === $relation) {
            throw new NotFoundHttpException('CUSTOMER_STORE_RELATION_NOT_FOUND');
        }

        $relation->setFavorite($data->isFavorite);
