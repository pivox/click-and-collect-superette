...
Limites Sprint 5 actuel : S5-001, S5-002, S5-003, S5-004 et S5-005 sont livrés. Limites connues de S5-003 : la génération de slug n'est pas protégée contre une race condition (deux créations simultanées du même nom). Le risque opérationnel est faible (admin seulement, faible concurrence) ; une contrainte `UNIQUE` en base sur `slug` est recommandée avant passage en production à charge élevée. S5-005 retourne un contrat QR scannable JSON et régénère le token opaque, mais ne génère pas d'image PNG/PDF côté backend. Le backend ne couvre pas encore l'onboarding guidé, l'email d'invitation, le billing, l'analytics, l'upload ou le CSV export.
