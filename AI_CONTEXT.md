...
- Sprint 5 : en cours côté backend. S5-001 PR #103 livrée (lecture admin comptes marchands : `GET /api/admin/merchants`, `GET /api/admin/merchants/{merchantId}`). S5-002 PR #104 livrée (lecture admin supérettes : `GET /api/admin/stores`, `GET /api/admin/stores/{storeId}`). S5-004 PR #105 livrée (mutations admin comptes marchands : `POST /api/admin/merchants`, `PATCH /api/admin/merchants/{id}`, `PATCH .../suspend`, `PATCH .../activate`). Prochaine étape recommandée : S5-003 — création/mise à jour admin des supérettes.

## Avancement global

- Backend MVP : environ 92 %.
- Produit terrain testable : environ 95 %.
- Sprint courant : Sprint 5 — Administration minimale.
- Prochaine PR recommandée : S5-003 — création/mise à jour admin des supérettes.
...
Limites Sprint 5 actuel : S5-001, S5-002 (lecture) et S5-004 (mutations marchands) sont livrés. Le backend ne couvre pas encore la création/modification/suspension de supérettes (S5-003), l'association marchand-supérette, la régénération QR admin, l'onboarding guidé, l'email d'invitation, le billing, l'analytics, l'upload ou le CSV export.
