...
- Backend MVP : 100 % (Sprint 7 audité et clôturé — S7-008 livré).
- Produit terrain testable : environ 95 % — parcours client complet validé par simulation Playwright (rapport `docs/qa/client-journey-simulation-report-v2.md`).
...
- Frontend client livré : parcours inscription → catalogue → Kadhia → suivi commande fonctionnel (8 issues QA v1 corrigées, 1 bug résiduel corrigé en S7-008).
- Non livré Sprint 7 : US-059 (PWA), US-060 (WCAG), US-008 (i18n AR câblé dans l'app) — reportés post-MVP.
- Prochaine priorité recommandée : transport Messenger persistant + Supervisor (R2 bloquant production), ou Sprint 8 frontend (PWA, AR, WCAG).
...
Limites Sprint 7 : S7-004 — `ip_address` dans `admin_audit_logs` n'est fiable que si `trusted_proxies` est configuré côté Symfony ; sinon peut contenir l'IP du proxy. Extension PostgreSQL `unaccent` requise par PR #152 (recherche accent-insensitive) mais aucune migration de création fournie — à ajouter manuellement (`CREATE EXTENSION IF NOT EXISTS unaccent;`) avant déploiement ou après recreate de la base. Transport Messenger `sync://` en local — les `DelayStamp` ne persistent pas au redémarrage ; un transport Doctrine/Redis + worker Supervisor sont requis en production pour les automatisations différées. Table `product_import_raw` orpheline en base (entité supprimée depuis Sprint 5 mais table toujours présente) — drift cosmétique non bloquant, à nettoyer avant production. PWA, WCAG et i18n AR câblé : non livrés dans le périmètre Sprint 7 backend-only. Frontend client : `GET /api/me/stores/{shopId}/kadhias` retourne 405 (endpoint existant en POST seulement) — la Kadhia s'affiche vide sur la page catalogue d'un client déjà connecté ; correctif reporté.
